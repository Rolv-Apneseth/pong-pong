...
def game_loop(window, WIDTH, HEIGHT):
    ...
    def redraw_window(window):
        ...
        # Displays win label
        if win:
            win_label = win_label_font.render(
                f"{who_won} won the game!", 1, (255, 255, 255)
            )

            window.blit(
                win_label, (WIDTH // 2 - win_label.get_width() // 2, HEIGHT // 2)
            )
    ...
    while running:
        ...
        if (
            keys[pygame.K_d] and player1.x + PADDLE_VELOCITY + player1.width < WIDTH
        ):  # right
            player1.move(PADDLE_VELOCITY)
        ...
        if (
            keys[pygame.K_RIGHT] and player2.x + PADDLE_VELOCITY + player2.width < WIDTH
        ):  # right
            player2.move(PADDLE_VELOCITY)
        ...
        for ball in balls:
            ...
            if (
                ...
                and player1.x < ball.x < player1.x + player1.width
            ):
                ball.yvel *= -1
            if (
                ball.y + ball.radius > player2.y
                and player2.x < ball.x < player2.x + player2.width
            ):
                ball.yvel *= -1
